...
function validateRow(row: MemberForm, knownMemberIds: Set<string>) {
  ...
  if (!row.full_name.trim() || row.full_name.trim().length < 2) errors.push("full_name must be at least 2 characters");
...
export default function AdminMembers() {
  ...
  const saveMember = async (e: SyntheticEvent<HTMLFormElement>) => {
    ...
    const eventType: N8nMembersEventPayload["event_type"] = editingId ? "member_updated" : "member_created";
    ...
    if (!webhookResult.ok && !webhookResult.skipped) {
      ...
      setMessage(
        `${editingId ? "Member updated" : "Member created"}. n8n webhook is not configured.`
      );
    } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={saveMember} className="rounded-2xl bg-black/35 p-6 space-y-3">
        <h2 className="text-xl font-black text-white">{editingId ? "Edit" : "Add"} Member</h2>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="space-y-1">
            <FieldLabel>Member ID</FieldLabel>
            <input className="w-full rounded-xl bg-white/10 px-4 py-3 text-white" value={form.member_id} onChange={(e) => onChange("member_id", e.target.value)} required />
          </div>
          <div className="space-y-1">
            <FieldLabel>Full Name</FieldLabel>
            <input className="w-full rounded-xl bg-white/10 px-4 py-3 text-white" value={form.full_name} onChange={(e) => onChange("full_name", e.target.value)} required />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="space-y-1">
            ...
            <input type="email" className="w-full rounded-xl bg-white/10 px-4 py-3 text-white" value={form.email || ""} onChange={(e) => onChange("email", e.target.value)} />
          </div>
          <div className="space-y-1">
            <FieldLabel>Phone</FieldLabel>
            <input className="w-full rounded-xl bg-white/10 px-4 py-3 text-white" value={form.phone} onChange={(e) => onChange("phone", e.target.value)} required />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="space-y-1">
            ...
            <select className="w-full rounded-xl bg-white/10 px-4 py-3 text-white" value={form.membership_type} onChange={(e) => onChange("membership_type", e.target.value as MembershipType)}>
              {membershipTypes.map((v) => <option key={v} value={v} className="bg-black">{v}</option>)}
            </select>
          </div>
          <div className="space-y-1">
            <FieldLabel>Membership Status</FieldLabel>
            <select className="w-full rounded-xl bg-white/10 px-4 py-3 text-white" value={form.membership_status} onChange={(e) => onChange("membership_status", e.target.value as MembershipStatus)}>
              {membershipStatuses.map((v) => <option key={v} value={v} className="bg-black">{v}</option>)}
            </select>
          </div>
          <div className="space-y-1">
            <FieldLabel>Payment Status</FieldLabel>
            <select className="w-full rounded-xl bg-white/10 px-4 py-3 text-white" value={form.payment_status} onChange={(e) => onChange("payment_status", e.target.value as PaymentStatus)}>
              {paymentStatuses.map((v) => <option key={v} value={v} className="bg-black">{v}</option>)}
            </select>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="space-y-1">
            ...
            <input type="date" className="w-full rounded-xl bg-white/10 px-4 py-3 text-white" value={form.start_date || ""} onChange={(e) => onChange("start_date", e.target.value)} />
          </div>
          <div className="space-y-1">
            <FieldLabel>End Date</FieldLabel>
            <input type="date" className="w-full rounded-xl bg-white/10 px-4 py-3 text-white" value={form.end_date || ""} onChange={(e) => onChange("end_date", e.target.value)} />
          </div>
          <div className="space-y-1">
            <FieldLabel>Last Visit Date</FieldLabel>
            <input type="date" className="w-full rounded-xl bg-white/10 px-4 py-3 text-white" value={form.last_visit_date || ""} onChange={(e) => onChange("last_visit_date", e.target.value)} />
          </div>
          <div className="space-y-1">
            <FieldLabel>Payment Due Date</FieldLabel>
            <input type="date" className="w-full rounded-xl bg-white/10 px-4 py-3 text-white" value={form.payment_due_date || ""} onChange={(e) => onChange("payment_due_date", e.target.value)} />
          </div>
        </div>

        <div className="space-y-1">
          <FieldLabel>Notes</FieldLabel>
          <textarea className="w-full rounded-xl bg-white/10 px-4 py-3 text-white" value={form.notes || ""} onChange={(e) => onChange("notes", e.target.value)} />
        </div>
      ...
      <div className="rounded-2xl bg-black/35 p-6 space-y-3">
        ...
        {members.length === 0 ? <p className="text-sm text-zinc-400">No members found.</p> : null}
        {members.map((row) => (
          <div key={row.id} className="rounded-xl bg-white/6 p-4">
            <div className="text-white font-bold">{row.full_name}</div>
            <div className="text-xs text-zinc-300 mt-1">
              {row.member_id} | {row.phone} | {row.email || "no-email"}
            </div>
            <div className="text-xs text-zinc-400 mt-1">
              {row.membership_type} / {row.membership_status} | payment: {row.payment_status}
            </div>
            <div className="mt-3 flex gap-2">
              <button type="button" onClick={() => onEdit(row)} className="rounded-lg bg-white/10 px-3 py-1 text-white">Edit</button>
              <button type="button" onClick={() => onDelete(row.id)} className="rounded-lg bg-red-500/20 px-3 py-1 text-red-300">Delete</button>
            </div>
          </div>
        ))}
      </div>
