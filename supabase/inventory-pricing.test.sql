...
begin;
do $$ begin
  if current_database() not like 'gym_pricing_test_%' then raise exception 'Disposable test database required'; end if;
end $$;
...
do $$
declare p bigint; v bigint; manual_id bigint; auto_id bigint; wholesale_id bigint; draft_id bigint; result public.invoices;
begin
  ...
  exception when check_violation then null; end;
  insert into public.invoices(customer_name,invoice_number,billing_date) values('Manual',' GYM-2099-000001 ','2099-01-01') returning id into manual_id;
  ...
  if result.invoice_number<>'GYM-2099-000001' or result.payment_status<>'partially_paid' or result.balance_minor<>1000 then raise exception 'Manual number or payment regression'; end if;
  begin
    insert into public.invoices(customer_name,invoice_number) values('Duplicate','gym-2099-000001');
  ...
  if result.invoice_number<>'GYM-2099-000002' then raise exception 'Automatic numbering did not skip reserved number'; end if;
  ...
  exception when raise_exception then
    if sqlerrm<>'An assigned bill number cannot be changed' then raise; end if;
  end;
