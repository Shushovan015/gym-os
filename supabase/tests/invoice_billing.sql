-- Run ONLY in a disposable schema clone with both 20260921 migrations applied.
-- Sequence tests intentionally consume numbers, even when a transaction aborts.
\set ON_ERROR_STOP on
do $$ begin
  if current_database() not like 'gym_changes_test%' then raise exception 'Disposable test database required'; end if;
end $$;
...
begin
  ...
  c:=public.claim_bill_email(900001,'{"to":["member@example.test"],"html":"original"}');
  if c->>'state'<>'claimed' then raise exception 'First send not claimed'; end if;
  if public.claim_bill_email(900001,'{}')->>'state'<>'busy' then raise exception 'Concurrent send not blocked'; end if;
  perform public.finish_bill_email(900001,(c->'delivery'->>'lease_token')::uuid,'uncertain');
  retry:=public.claim_bill_email(900001,'{"to":["changed@example.test"],"html":"changed"}');
  if retry->'delivery'->>'delivery_key' <> c->'delivery'->>'delivery_key' or retry->'delivery'->'payload' <> c->'delivery'->'payload' then raise exception 'Uncertain retry changed payload or idempotency key'; end if;
  perform public.finish_bill_email(900001,(retry->'delivery'->>'lease_token')::uuid,'sent','provider-test');
  if public.claim_bill_email(900001,'{}')->>'state'<>'sent' then raise exception 'Sent bill could be resent'; end if;
  c:=public.claim_bill_email(900002,'{"to":["member@example.test"]}');
  perform public.finish_bill_email(900002,(c->'delivery'->>'lease_token')::uuid,'uncertain');
  update public.invoice_email_deliveries set created_at=now()-interval '25 hours' where invoice_id=900002;
  if public.claim_bill_email(900002,'{}')->>'state'<>'review_required' then raise exception 'Expired uncertain send could be duplicated'; end if;
  if has_function_privilege('authenticated','public.claim_bill_email(bigint,jsonb)','EXECUTE') or has_function_privilege('anon','public.claim_bill_email(bigint,jsonb)','EXECUTE') then raise exception 'Client can claim sends directly'; end if;
