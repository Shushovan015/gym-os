-- Reconcile both branches: global automatic sequence + optional manual numbers.
-- Applied last on new installs and on installations upgraded from either branch.
...
begin
  -- An upsert runs INSERT triggers before the UPDATE path. Validate on UPDATE.
...
language plpgsql security definer set search_path=public as $$
declare serial bigint; prefix text; candidate text;
begin
  ...
  if new.invoice_status='issued' and new.invoice_number is null then
    ...
    loop
      ...
      candidate := prefix||'-'||extract(year from new.billing_date)::int||'-'||lpad(serial::text,greatest(6,length(serial::text)),'0');
      exit when not exists(select 1 from public.invoices where lower(btrim(invoice_number))=lower(candidate));
    end loop;
...
-- Retire the Resend-only RPCs; uncertain SMTP sends must not follow its retry logic.
drop function if exists public.claim_bill_email(bigint,jsonb);
drop function if exists public.finish_bill_email(bigint,uuid,text,text,text);
...
-- Both earlier migration orders now have all Gmail columns. Preserve all outcomes.
do $$ begin
